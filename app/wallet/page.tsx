...
import { browser, type WalletTransaction } from '@/lib/supabase/queries';
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-5 gap-4">
        <p className="text-ink-500">Connectez-vous pour voir votre portefeuille.</p>
        <Link
      ...
      <div className="mx-auto max-w-3xl px-5 sm:px-8 lg:px-12">
        ...
        >
          ...
          <p className="text-[14px] text-ink-400 mb-8">
            Vos gains de voyageur, en un coup d&apos;œil.
          </p>
            ...
            <div className="bg-gradient-to-br from-ink-500 to-ink-600 rounded-3xl p-7 lg:p-9 text-cream-50 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-[13px] text-cream-50/70">
                  {captured.length} {captured.length > 1 ? 'transactions encaissées' : 'transaction encaissée'}
                </p>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-[12px] text-cream-50/60 leading-relaxed pt-1">
                        Les retraits bancaires arrivent bientôt. En attendant, contactez-nous à{' '}
                        <a
                          href="mailto:hello@jibly.com?subject=Retrait%20de%20mes%20gains"
                          className="underline font-semibold hover:text-cream-50 transition-colors"
                        >
                          hello@jibly.com
                        </a>{' '}
                        pour recevoir vos gains.
                      </p>
                    </div>
                  </motion.div>
                )}
            ...
            {authorized.length === 0 ? (
              ...
              <>
                <div className="bg-white rounded-2xl px-5 py-4 border border-ink-50 mb-3">
                  <div className="text-[12px] text-ink-400 mb-1">À recevoir après livraison</div>
          ...
          {incoming.length === 0 && !error && (
            <div className="bg-white rounded-3xl p-12 text-center border border-dashed border-ink-100 mt-6">
              ...
              <p className="text-[14px] text-ink-400 mb-6 max-w-sm mx-auto leading-relaxed">
                Publiez un trajet ou répondez à une demande de transport pour commencer à gagner.
              </p>
              <Link
                href="/voyager"
...
function TxRow({
  ...
  const senderName = displayName(tx.sender_profile?.full_name) || 'Expéditeur';
  ...
  const statusStyle =
    kind === 'captured'
      ? { color: 'text-mint-600', icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: 'Encaissé' }
      : kind === 'pending'
        ? { color: 'text-butter-500', icon: <Clock className="w-3.5 h-3.5" />, label: 'En attente' }
        : { color: 'text-ink-300', icon: <XCircle className="w-3.5 h-3.5" />, label: 'Annulé' };
